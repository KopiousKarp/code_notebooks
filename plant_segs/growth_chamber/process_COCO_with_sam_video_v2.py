...
def process_with_sam2(coco_root, coco_api, sam2_checkpoint, model_cfg, device="cuda"):
    ...
    folder_segment_start_ideces = []
    for img_id in img_ids:
        if filename not in frame_names:
            continue
        cond_frame_idx = frame_names[coco_api.loadImgs(img_id)[0]['file_name']]
        folder_segment_start_ideces.append(cond_frame_idx)
    folder_segment_start_ideces.sort()
    if len(folder_segment_start_ideces) > 0:
        for i in range(len(folder_segment_start_ideces)-1):
            segment_idxs = list(range(folder_segment_start_ideces[i], folder_segment_start_ideces[i+1]))
            #make a folder segment
            segment_video_dir = tempfile.mkdtemp()
            for i, idx in enumerate(segment_idxs):
                shutil.copy(os.path.join(temp_video_dir, f"{idx:04d}.jpg"), os.path.join(segment_video_dir, f"{i:04d}.jpg"))
            segment_inference_state = predictor.init_state(video_path=segment_video_dir,
            ...
            for img_id in img_ids:
                if filename not in frame_names:
                    continue
                cond_frame_idx = frame_names[coco_api.loadImgs(img_id)[0]['file_name']]
                if cond_frame_idx not in segment_idxs:
                    continue
                ...
                for ann in anns:
                    ...
                    _, out_obj_ids, _ = predictor.add_new_mask(
                        inference_state=segment_inference_state,
                        frame_idx=segment_idxs.index(cond_frame_idx),
                        obj_id=ann['category_id'],  # Use annotation ID as object ID
                        mask=mask_tensor
                    )
            # Propagate masks through all frames
            for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(segment_inference_state):
                video_segments[segment_idxs[out_frame_idx]] = {
                    out_obj_id: (out_mask_logits[i] > 0.0).cpu().numpy()
                    for i, out_obj_id in enumerate(out_obj_ids)
                    }
            del segment_inference_state
            shutil.rmtree(segment_video_dir)
                
    else:
        print("There are not multiple segments in the video based on the COCO annotations.")
